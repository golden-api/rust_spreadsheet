...
impl SpreadsheetApp {
    ...
    fn update_selected_cell(&mut self) {
        ...
        self.status_message = match unsafe { STATUS_CODE } {
            0 => format!(
                "Updated cell {}{}",
                col_label(c), // uses the new col_label function
                r + 1
            ),
...
impl eframe::App for SpreadsheetApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        CentralPanel::default().show(ctx, |ui| {
            // =========================
            // 1) Top Area: Formula Bar
            // =========================
            Frame::none()
                .fill(self.style.header_bg)
                // reduce the margin to tighten up vertical space
                .inner_margin(5.0)
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        // Formula Input
                    ...
                    // (Removed the up/down/left/right buttons here)

                    // Status message, placed inline, no extra space
            ...
            // =========================
            // 2) Scrollable Spreadsheet
            // =========================
            ScrollArea::both()
                .id_source("spreadsheet_scroll_area")
                .drag_to_scroll(true)  // ensure both scrollbars are visible
                .auto_shrink([false, false]) // don't auto-shrink
                .show(ui, |ui| {
                    // Column headers
                    ui.horizontal(|ui| {
                        // Empty cell for the row/col intersection
                        ui.add_sized(
                                ...
                                for j in 0..visible_cols {
                                    ...
                                    if col < self.sheet[row].len() {
                                        ...
                                        if is_selected && self.editing_cell {
                                            // Editing mode
                                            Frame::none()
                                                .fill(Color32::from_rgb(40, 40, 70))
                                                .show(ui, |ui| {
            ...
            // =========================
            // 3) Selected Cell Display
            // =========================
            ui.add_space(5.0);
            let (row, col) = self.selected;
            let col_letter = col_label(col);
            ui.label(
        ...
        // Process any new selection after the UI code
