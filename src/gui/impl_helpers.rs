...
impl SpreadsheetApp {
    ...
    pub fn update_selected_cell(&mut self) {
        ...
        if let Some((r, c)) = self.selected {
            ...
            parser::update_and_recalc(&mut self.sheet,&mut self.ranged,&mut self.is_range, total_rows, total_cols, r, c, old_cell);
...
impl SpreadsheetApp {
    ...
    pub fn undo(&mut self) {
        if let Some(action) = self.undo_stack.pop() {
            ...
            let current_cell = self.sheet.get(&idx).cloned().unwrap_or(Cell { value: Valtype::Int(0), data: CellData::Empty, dependents: HashSet::new() });
            ...
            *self.sheet.get_mut(&idx).unwrap() = action.old_cell;
            ...
            parser::update_and_recalc(&mut self.sheet,&mut self.ranged,&mut self.is_range ,total_rows, total_cols, row, col, current_cell);
            ...
            if let Some(copied_cell) = clipboard_data {
                ...
                if !clipboard_formula_copy.is_empty() {
                    ...
                    *self.sheet.get_mut(&idx).unwrap() = copied_cell;
                    // Recalculate dependencies
                    parser::update_and_recalc(&mut self.sheet, &mut self.ranged,&mut self.is_range,total_rows, total_cols, row, col, old_cell);
                }
